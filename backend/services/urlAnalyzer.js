...
const { detectHomograph } = require('./homographDetector');
...
function analyzeURL(url) {
  ...
  // parsedURL.hostname is already Punycode-normalised by the WHATWG URL parser.
  // For homograph detection we need the raw (potentially Unicode) hostname from
  // the original string before that normalisation happens.
  const rawHostnameMatch = url.match(/^https?:\/\/([^/?#:]+)/i);
  const rawHostname = rawHostnameMatch ? rawHostnameMatch[1] : parsedURL.hostname;
  ...
  // 9. IDN Homograph attack detection — use raw hostname (pre-Punycode)
  const homograph = detectHomograph(rawHostname);
  if (homograph.detected) {
    if (homograph.impersonates) {
      // Deliberate brand impersonation via Unicode lookalikes → very high penalty
      score += 40;
      reasons.push(
        `IDN homograph attack: domain uses Unicode characters to impersonate "${homograph.impersonates}"` +
        (homograph.scripts.length > 0 ? ` (${homograph.scripts.join(', ')} script)` : '')
      );
    } else {
      // Mixed scripts without a brand match — still suspicious
      score += 20;
      reasons.push(
        `Mixed Unicode scripts in domain (${homograph.scripts.join(', ')}) — possible homograph attack`
      );
    }
  }
  ...
  return { risk_score, classification, reasons, homograph };
}
